...
2- given an array of integer find the most repeated number
	findMostRepeated([2,2,3,3,4,4,4,1,1,1,1,1,6,6,6,1,1,4,4,4,6,4,4,4]) = 4
3- given an array of integer find the number with the longest repeating sequence
	findMostRepeated([2,2,3,3,4,4,4,1,1,1,1,1,6,6,6,1,1,4,4,4,6,4,4,4]) = 1
4- Count Total number of zeros from 1 upto n
	countZero(2014) = 223
	countZero(100) = 11
5- create all permutations of a string


Class Inheritance: 
instances inherit from classes (like a blueprint — a description of the class), and create sub-class relationships: hierarchical 
class taxonomies. Instances are typically instantiated via constructor functions with the `new` keyword. Class inheritance may 
or may not use the `class` keyword from ES6.
...
const findLongSeq = (array) => {
	...
	for(let key in freqCounter) {		
		if(freqCounter[key] > maxVal){
			maxNum = key
			maxVal = freqCounter[key]
		} 
	}
...
const findMostRepeated = (array) => {
	...
	array.forEach(ele => {
		freqCounter[ele] = ( freqCounter[ele] || 0 ) + 1
	})
